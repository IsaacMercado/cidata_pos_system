    ...
    def _onchange_checkin_date(self):
        ...
        if self.checkin_date and self.checkout_date and self.checkout_date <= self.checkin_date:
            raise ValidationError(
                _("Checkout must be greater than checkin date"))
        if self.checkin_date and self.checkout_date:
            # Worker/UI contracts use checkout as an exclusive boundary:
            # 2026-09-01 -> 2026-09-04 is three nights.
            self.uom_qty = (self.checkout_date - self.checkin_date).days

    @api.constrains("checkin_date", "checkout_date")
    def _check_booking_dates(self):
        for line in self:
            if line.checkin_date and line.checkout_date and line.checkout_date <= line.checkin_date:
                raise ValidationError(
                    _("Checkout must be greater than checkin date"))
